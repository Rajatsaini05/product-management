...
// ✅ Setup transporter for Amazon WorkMail
const transporter = nodemailer.createTransport({
  host: process.env.SMTP_HOST, // smtp.mail.us-east-1.awsapps.com
  port: Number(process.env.SMTP_PORT), // 465 or 465
  secure: Number(process.env.SMTP_PORT) === 465, // true if SSL, false if TLS
  auth: {
    user: process.env.EMAIL_USERNAME, // WorkMail login
...
export const signin = async (req, res, next) => {
  try {
    ...
    if (!user) return next(createError(201, "User not found"));

    if (user.googleSignIn) {
      return next(
        createError(
          201,
          "Entered email is signed up with Google. Please sign in with Google."
        )
      );
    }

    const validPassword = bcrypt.compareSync(req.body.password, user.password);
    if (!validPassword) return next(createError(201, "Wrong password"));
...
export const generateOTP = async (req, res, next) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL, // must be WorkMail address
      to: email,
      subject:
      ...
      html:
        reason === "FORGOTPASSWORD"
          ? `<p>Dear ${name},</p>
             ...
             <p>Your OTP is: <b>${req.app.locals.OTP}</b></p>`
          : `<p>Dear ${name},</p>
             <p>Welcome to VEXA! Please verify your account.</p>
             <p>Your OTP is: <b>${req.app.locals.OTP}</b></p>`,
    };
...
export const resetPassword = async (req, res, next) => {
  ...
  try {
    const user = await User.findOne({ email });
    if (!user) return res.status(202).send({ message: "User not found" });
