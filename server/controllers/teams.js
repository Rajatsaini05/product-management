  ...
  try {
    ...
    await Promise.all(
      team.members.map(async (Member) => {
        if (Member.id.id === req.user.id) verified = true;
      })
    ).then(() => {
      if (verified) {
...
export const updateTeam = async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < Team.members.length; i++) {
      ...
      if (Team.members[i].id.toString() === req.user.id) {
        if (
          Team.members[i].access === "Owner" ||
          Team.members[i].access === "Admin" ||
          Team.members[i].access === "Editor"
        ) {
          ...
          return next(
            createError(403, "You are not allowed to update this Teams!")
          );
        }
      }
    }
    return next(
      createError(403, "You can update only if you are a member of this Teams!")
    );
  } catch (err) {
    next(err);
  }
    ...
    if (code === req.app.locals.CODE) {
      ...
      for (let i = 0; i < team.members.length; i++) {
        if (team.members[i].id.toString() === user.id) {
          return next(
            createError(403, "You are already a member of this team!")
          );
        }
      }
